...
契约用 TypeScript 定义。V0.4.3 引入 Pydantic AI 时，字段一一对应转成 Pydantic 模型，调用方不改。
...
LangGraph 解决的是另一类问题——需要停下来等用户、按状态分支、断点恢复。V0.4.2 的初始诊断已经包含等待用户，但当前只需要一个简单暂停点，先用数据库状态和幂等 API 实现；V0.4.3 再把它迁移成 `interrupt` 与 checkpoint。以下场景同时出现后，图编排的收益才明显：
