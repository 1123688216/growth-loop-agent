...
> 修订日期：2026-09-01
>
> 当前状态：V0.4.3 的工程 MVP 与基础课件阅读器已完成；V0.4.4 已实现个人资料库、Schema V10、MinerU GPU 解析、结构化原子节点、不可变 ParseRun/ChunkSet、父子切片、FTS5、文本 Embedding 和资料级向量检索。目标范围 Agentic RAG Tool、可恢复课程编排、课程/题目来源绑定与固定快照评分尚未实现。
...
- 数据库当前为 V10：在课程内容版本、质量报告、教学块来源骨架和学习作息之外，已加入真实资料、不可变版本、ParseRun、DocumentNode、不可变 ChunkSet、Parent/Child、FTS5、目标关系、检索运行快照、题目来源、Embedding Profile/Run、Chunk Embedding 和 Pipeline 阶段计时；当前资料级向量检索可以写入 RetrievalRun，但目标范围 Tool 与课程引用关系仍待接通。
...
RAG 本身不是一个独立 Agent。文件解析、分块、索引、权限过滤、检索和快照校验属于确定性服务；后续把它们封装成 `search_knowledge_base / read_source_chunks / check_source_coverage` 等窄 Tool，由 Planner/Tutor 根据当前任务决定何时调用。Tool 调用必须记录检索运行并继承服务端用户/目标上下文，这里的 “Agentic” 指 Agent 决定调用策略，不是给 RAG 再造一个角色。
...
接入时点已经调整：V0.4.3 的 `LearningBlock`、课程版本和质量门禁已经稳定，V0.4.4 的 SourceVersion、ChunkSet、RetrievalRun 和 EvidenceBundle 关系也已冻结；而接下来的工作会直接修改课程生成，加入诊断等待、资料覆盖分支、来源不足等待和质量修复循环。若继续把这些逻辑写入现有 Node 手写流程，再迁移 LangGraph，会重复重构同一条主链。因此在目标资料 Tool 完成后、Tutor 接入 EvidenceBundle 之前，先迁移一条窄范围可恢复课程主图。

本次提前的是**课程主流程编排**，不是全项目框架重写。解析、切片、Embedding、检索计算、权限、评分归一化、掌握度更新和业务数据库事务继续保持确定性服务边界。
...
### V0.4.4：私有资料 RAG、可恢复编排与可追溯教学来源

详细范围、Schema V8-V10、数据契约、安全边界和验收标准见 [V0.4.4 实施方案](IMPLEMENTATION_PLAN_V044.md)。当前剩余阶段按以下顺序推进：

1. 支持 PDF、DOCX、TXT 和粘贴文本；
2. 建立不可变资料版本、结构化原子节点、不可变父子切片、FTS5 和文本 Embedding；
3. 完成目标资料绑定和可审计 Agentic RAG Tool；
4. 在修改 Tutor 前建立 LangGraph checkpoint/interrupt 与 Pydantic AI 结构化节点；
5. 课程必要教学块绑定真实来源片段；
6. 题目、参考答案和 rubric 绑定同一来源快照；
7. 评分不重新检索，历史标准不随资料更新改变；
8. 资料不足时进入可恢复的 `source_insufficient` interrupt；
9. 页面展示持久化进度、来源标题、页码/章节和引用片段。

文本 Embedding 已完成 MVP。用户指定 URL 和联网可信来源继续放在本地资料、可恢复编排、来源课程和固定评分闭环之后，避免联网失败污染本地课程主链。

### V0.4.5：工作流扩展与加固

1. 在 V0.4.4 最小课程主图上扩展补课和变式重测循环；
2. 增加间隔复习、长时间任务和更多人工确认点；
3. 加固并发、取消、超时、死信、工作流版本迁移和 checkpoint 清理；
4. 建立节点级成功率、重试率、恢复率、P50/P95 耗时和 Token 报表；
5. 保持 Next.js 为数据库唯一业务写入方，浏览器不直接访问 Python 服务；
6. 普通课堂提问继续作为 Tutor 子流程，不重跑课程创建主图。
